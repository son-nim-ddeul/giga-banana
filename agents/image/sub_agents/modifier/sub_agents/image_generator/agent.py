...
image_generator_agent = LlmAgent(
    ...
    instruction="""
    1. 유저와의 대화에서 워크플로우를 생성하기 위해 필요한 정보를 파악합니다.
    ...
    4. 워크플로우 생성 가이드와 **워크플로우 최적화 노드** 문서(파이프라인·유틸·메타데이터·패턴)를 참고하여 워크플로우를 구성·최적화합니다.
    5. 워크플로우의 프롬프트 작성 후 원본 이미지의 프롬프트(태그)를 추가합니다.
    6. 워크플로우 생성 후 이미지를 생성합니다.
    """,
    tools=[
        read_workflow_creation_guide,
        read_optimization_nodes_info,
