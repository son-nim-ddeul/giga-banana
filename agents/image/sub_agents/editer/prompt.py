...
### 1단계: 요청 분석 및 검증
- **이미지 첨부 확인**: 사용자가 수정할 이미지를 업로드했는지 확인
  - edit_image 도구가 최근 업로드된 이미지를 자동으로 감지
...
- edit_image는 세션에서 최근 업로드된 이미지를 자동으로 감지하여 사용
- 생성된 수정 프롬프트(edit_prompt)와 필요시 aspect_ratio, image_size만 전달
...
- **이미지 업로드 필수**: 사용자가 수정할 이미지를 업로드해야 함
- **자동 이미지 감지**: edit_image 도구가 세션의 최근 업로드 이미지를 자동으로 찾아서 사용
